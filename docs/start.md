...
OPTIONS
  ...
  -t, --target=target
      [default: target] The target folder where the gcr-node network is generated
...
EXAMPLES
  $ gcr-node start
  $ gcr-node start -p bootstrap
  $ gcr-node start -p testnet -a dual
  $ gcr-node start -p testnet -a dual --password 1234
  $ echo "$MY_ENV_VAR_PASSWORD" | gcr-node start -p testnet -a dual
```

_See code: [src/commands/start.ts](https://github.com/goldcoinreserve/gcr-node/blob/v1.0.4/src/commands/start.ts)_
