...
export class VotingService {
    ...
    public async run(
        ...
        if (!update && currentVotingFiles.length > 0) {
            logger.warn('');
            logger.warn(`Voting key files are close to EXPIRATION or have EXPIRED!. Run the 'gcr-node updateVotingKeys' command!`);
            logger.warn('');
            return false;
        }
        ...
        if (nemesisBlock) {
            ...
            logger.warn(`For linking, you can use 'gcr-node link' command, the symbol cli, or the symbol desktop wallet.`);
            logger.warn('');
        }
