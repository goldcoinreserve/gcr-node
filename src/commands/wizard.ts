...
export default class WizardCommand extends Command {
    ...
    static examples = [`$ gcr-node wizard`];
...
export class Wizard {
    ...
    public async execute(flags: {
        ...
        this.logger.info('Welcome to the Symbol gcr-node wizard! This command will:');
        ...
        if (existsSync(target)) {
            throw new Error(
                'There is currently a ./target folder here!!!! Have you executed gcr-node already? You should move the folder somewhere else before overwriting it!',
            );
        }
        ...
        if (network == Preset.bootstrap) {
            ...
            this.logger.info('');
            this.logger.info(`$ gcr-node start -p ${preset} -a ${assembly}`);
            return;
        }
        ...
        if (
            ...
            !(
                await prompt([
                    {
                        name: 'offlineNow',
                        message: `gcr-node is about to start working with sensitive information (private keys) so it is highly recommended that you disconnect from the network before continuing. Say YES if you are offline or if you don't care.`,
                        type: 'confirm',
                        default: true,
                    },
                ])
        ...
        this.logger.info('');
        this.logger.info(
            'gcr-node needs to provide the node with a number of key pairs (Read more at https://docs.symbolplatform.com/concepts/cryptography.html#symbol-keys).',
        );
        this.logger.info(`If you don't know what a key is used for, let gcr-node generate a new one for you.`);
        ...
        this.logger.info('');
        this.logger.info('');
        this.logger.info(`The Symbol gcr-node preset file '${customPresetFile}' has been created!!!. Keep this safe!`);
        this.logger.info('');
        this.logger.info(`To decrypt the node's private key, run: `);
        this.logger.info('');
        this.logger.info(`$ gcr-node decrypt --source ${customPresetFile} --destination plain-custom-preset.yml`);
        this.logger.info('');
        this.logger.info('Remember to delete the plain-custom-preset.yml file after used!!!');

        this.logger.info(
            `You can edit this file to further customize it. Read more https://github.com/symbol/gcr-node/blob/main/docs/presetGuides.md`,
        );
        this.logger.info('');
        this.logger.info(
        ...
        this.logger.info(`$ gcr-node start -c ${customPresetFile} ${targetParam}`);
        ...
        this.logger.info('');
        this.logger.info(`$ gcr-node pack -c ${customPresetFile} ${targetParam}`);
        this.logger.info('');
        this.logger.info(
            `Once the target folder is created, gcr-node will use the protected and encrypted addresses.yml, and preset.yml in inside the target folder.`,
        );
        this.logger.info(
        ...
        this.logger.info('');
        this.logger.info(`$ gcr-node link --useKnownRestGateways -c ${customPresetFile}`);
        this.logger.info('');
    }
            ...
            const responses = await prompt([
                {
                    ...
                    choices: [
                        ...
                        { name: 'gcr-node Local Network', value: Preset.bootstrap },
                        {
        ...
        const { mode } = await prompt([
            {
                ...
                choices: [
                    {
                        name:
                            'PROMPT_MAIN: gcr-node may ask for the Main private key when doing certificates upgrades. Other keys are encrypted.',
                        value: PrivateKeySecurityMode.PROMPT_MAIN,
                    },
                    {
                        name:
                            'PROMPT_MAIN_TRANSPORT: gcr-node may ask for the Main and Transport private keys when regenerating certificates. Other keys are encrypted. Recommended for most nodes',
