...
export default class Pack extends Command {
    ...
    static examples = [
        `$ gcr-node pack`,
        `$ gcr-node pack -p bootstrap -c custom-preset.yml`,
        `$ gcr-node pack -p testnet -a dual -c custom-preset.yml`,
        `$ gcr-node pack -p mainnet -a dual --password 1234 -c custom-preset.yml`,
        `$ echo "$MY_ENV_VAR_PASSWORD" | gcr-node pack -p mainnet -a dual -c custom-preset.yml`,
    ];
    ...
    public async run(): Promise<void> {
        ...
        if (
            ...
            !(
                await prompt([
                    {
                        name: 'offlineNow',
                        message: `gcr-node is about to start working with sensitive information (certificates and voting file generation) so it is highly recommended that you disconnect from the network before continuing. Say YES if you are offline or if you don't care.`,
                        type: 'confirm',
                        default: true,
                    },
                ])
        ...
        console.log(`$ gcr-node start -p ${preset}${assembly ? ` -a ${assembly}` : ''} -c ${configOnlyCustomPresetFileName}`);
    }
}
