...
export default class Config extends Command {
    ...
    static examples = [
        ...
        `$ gcr-node config -p mainnet -a peer -c custom-preset.yml`,
        `$ gcr-node config -p mainnet -a my-custom-assembly.yml -c custom-preset.yml`,
        `$ gcr-node config -p my-custom-network.yml -a dual -c custom-preset.yml`,
    ...
    static flags = {
        ...
        upgrade: flags.boolean({
            description: `It regenerates the configuration reusing the previous keys. Use this flag when upgrading the version of gcr-node to keep your node up to date without dropping the local data. Backup the target folder before upgrading.`,
            default: ConfigService.defaultParams.reset,
        }),
